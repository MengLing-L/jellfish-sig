...
mod tests {
    ...
    #[test]
    fn test_signature_circuit() -> Result<(), CircuitError> {
        test_signature_circuit_helper::<_, Param254>()
    }

    fn test_signature_circuit_helper<F, P>() -> Result<(), CircuitError>
    where
        F: RescueParameter,
        P: Config<BaseField = F>,
    {
        // let kernel = MultiKernel::create(include_bytes!("/Users/mengling/Project/jellyfish-tp/primitives/src/gpu/cl/lib.fatbin"));
        ...
        // Good path
        let (circuit, bit) = build_is_valid_signature_circuit(vk, &msg, &sig)?;
